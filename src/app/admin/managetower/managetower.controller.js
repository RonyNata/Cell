(function() {
  ...
  angular
    .module('cell')
    .controller('ManageTowerController', ManageTowerController);

  /** @ngInject */
  function ManageTowerController($timeout, webDevTec, toastr, $q, MainService, ManageTowerService, $log, $scope) {
    var vm = this;
    ...
    vm.save = function(){
      ...
      ManageTowerService.CreateDataBTS(vm.item).then(
        function(){
    ...
    vm.deleteCellBTS = function(selectedCellBts) {
        ManageTowerService.deleteCellBTS(selectedCellBts).then(
            function(response) {
                vm.item.latCellBts = null;
                vm.item.longCellBts = null;
                vm.item.radiusCellBts = null;
    ...
    vm.login = function(){
      ManageTowerService.Login(vm.loginData).then(
        function(response){
          // if(navigator.cookieEnabled)
            ManageTowerService.setCookie('cred', response.tokenType + ' ' + response.accessToken, 30);
          // else sessionStorage.setItem('cred', response.tokenType + ' ' + response.accessToken);
          isLogin();
        }, function(errResponse){
          ManageTowerService.showToastrFailed(errResponse.data.message);
        })
    }

    vm.logout = function() {
        // if(navigator.cookieEnabled) 
          ManageTowerService.deleteCookie();
    ...
    function isLogin(){
      ...
      var cred = ManageTowerService.checkCookie();
